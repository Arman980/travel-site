...
app.get('/getSites', (req, res) => {
  pool.connect(function (err, client) {
    ...
    client.query('SELECT * FROM sites', //do the query
      (err, resp) => {
        if (err) {
          queryError(res, err)
          return;
...
app.get('/getInfo', (req, res) => {
  ...
  pool.connect(function (err, client) {
    ...
    client.query(`SELECT * FROM sites WHERE id=${id};`, //do the query
      (err, resp) => {
        if (err) {
          queryError(res, err)
          return;
...
app.post('/postSite', async (req, res) => {
  ...
  let query = `INSERT INTO sites(title, description, images) VALUES ('${title}', '${description}', '${insertImages}') RETURNING id;`
  console.log(query)
